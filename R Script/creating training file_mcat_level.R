...
df<- readxl::read_excel(File1,sheet = 4)
...
PMCAT_TRAINING_LOC <- paste0(PMCAT_Testing_PATH,"pmcat_training2.txt")
PMCAT_TRAINING_LOC2 <- paste0(PMCAT_Testing_PATH,"pmcat_vlookup2.csv")
...
write.csv(data_unique,PMCAT_TRAINING_LOC2,row.names = F,quote = F)
...
dir.create(paste0(MCAT_Training_PATH,"vlookup_data/"),showWarnings = F)
dir.create(paste0(MCAT_Training_PATH,"Training Files/"),showWarnings = F)
for (k in 1:length(out2)) {
  hold <- unique(out2[[k]]$PMCAT2)
  Data_out <- out2[[k]]$Fasttext_label
  #LOCC2 <- paste0("E:\\Yarn-product classification\\27-09-2018\\33_PMCAT_Yarn\\Training_data_PMCAT/vlookup",hold,".csv")
  LOCC_h <- paste0(MCAT_Training_PATH,"vlookup_data/",hold,"vlookup.csv")
  write.csv(out2[[k]],LOCC_h,row.names = F,quote = F)
  #MCAT_Training_PATH <- paste0("E:\\Yarn_new\\Child Training and testing\\Training Files/")
  #LOCC <- paste0("E:\\Yarn_new\\Child Training and testing\\Training Files/",hold,".txt")
  LOCC <- paste0(MCAT_Training_PATH,"Training Files/",hold,".txt")
  write.table(Data_out,LOCC,sep = "\t",row.names = F,col.names = F,quote = F)
}

New_LOC <- paste0(MCAT_Training_PATH,"MCAT_vlookup.csv")
